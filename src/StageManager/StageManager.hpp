...
namespace Pipeline {
	...
	namespace Stage {
		...
		public:
			...
			StageManager(std::unordered_map<std::size_t, Stage::Stage<DataT>>&& _stagesMap,
			...
			virtual const std::unordered_map<std::size_t, Stage::Stage<DataT>>& getStageMap() const override {
				return stagesMap;
			}
